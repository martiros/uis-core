...
//        Request::file('image')->get
        ...
        try {
            ...
            $valResult->addError('result', trans(' media.img_uploader.error.invalid_max_size', null, [$e->getMaxSize()]));
        } catch (Media_ImgUploader_Exception_InvalidExtension $e) {
            $result->status = 'INVALID_DATA';
            $result->errors = ['result'    =>    trans(' media.img_uploader.error.invalid_image_type', null, [implode(', ', $e->getAllowedExtensions())])];
        } catch (Media_ImgUploader_Exception_InvalidMinWidth $e) {
            $result->status = 'INVALID_DATA';
            $result->errors = ['result'    =>    trans(' media.img_uploader.error.invalid_min_width', null, [$e->getMinWidth()])];
        } catch (Media_ImgUploader_Exception_InvalidMinHeight $e) {
            $result->status = 'INVALID_DATA';
            $result->errors = ['result'    =>    trans(' media.img_uploader.error.invalid_min_height', null, [$e->getMinHeight()])];
        } catch (Media_ImgUploader_Exception_InvalidMaxWidth $e) {
            $result->status = 'INVALID_DATA';
            $result->errors = ['result'    =>    trans(' media.img_uploader.error.invalid_max_width', null, [$e->getMaxWidth()])];
        } catch (Media_ImgUploader_Exception_InvalidMaxHeight $e) {
            $result->status = 'INVALID_DATA';
            $result->errors = ['result'    =>    trans(' media.img_uploader.error.invalid_max_height', null, [$e->getMaxHeight()])];
        }
